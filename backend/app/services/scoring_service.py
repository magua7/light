class ScoringService:
    ...
    def calculate_scores(self, ai_result: dict, satellite_info: dict, ecology_info: dict) -> dict:
        ...
        blue_risk = bool(ai_result.get("summary", {}).get("blue_risk", False))
        type_count = ai_result.get("summary", {}).get("type_count", {})
        suggestions = self.build_suggestions(level, type_count, blue_risk, ecology_info)
    ...
    def build_suggestions(
        ...
        if type_count.get("stay_light", 0) >= 3:
            suggestions.append("建议对常亮景观灯实施分区控制，优先关闭低价值照明。")
        if blue_risk:
