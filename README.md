...
一个适合大学生计算机设计大赛 / 毕业设计展示的前后端分离项目，用于完成城市夜间光污染样本的上传、分析、评级、地图展示与预警管理。
...
- AI：通过独立 `ai_service.py` 提供 mock 分析能力，便于后续替换为真实模型服务
- 演示数据：默认生成湖南长沙范围的样本点位
...
后续若接入真实模型服务，建议只替换该服务层，不直接改动路由和页面结构。
